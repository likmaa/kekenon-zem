...
const getEnv = (key) => {
  ...
  try {
    ...
    console.warn(`[app.config.js] Erreur lors de la lecture manuelle du .env pour ${key}:`, err);
  }
  return undefined;
};
...
if (!MAPBOX_DOWNLOAD_TOKEN) {
  throw new Error(
    "[kekenon-zem] MAPBOX_DOWNLOAD_TOKEN manquant. Créez un secret EAS (eas secret:create) ou ajoutez-le dans .env — voir env.example."
  );
}
...
module.exports = ({ config: _config }) => config;
